...
const LevelsScreen = () => {
  ...
  const sideSpacer = useMemo(
    () => Math.max(0, width / 2 - ITEM_WIDTH / 2), // без GAP
    [width]
  )
  ...
  useEffect(() => {
    const assets = [
      backgroundImage,
      lockIcon,
      ...Array.from({ length: TOTAL_LEVELS }, (_, i) => getLevelIcon(i + 1)),
    ]
    Promise.all(assets.map((m) => Asset.fromModule(m).downloadAsync())).catch(
      () => {}
    )
